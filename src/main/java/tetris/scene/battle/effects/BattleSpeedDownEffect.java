 ...
 * 배틀 모드 전용 속도 감소 효과
 */
public class BattleSpeedDownEffect implements BattleItemEffect {
    private static final long EFFECT_DURATION = 5000; // 5초
    private static final double SPEED_DOWN_VALUE = 1500.0; // 1500ms로 설정
    ...
    public void activate(BattleItemContext context) {
        ...
        System.out.println("🐌 SPEED DOWN: " + sourcePlayer.getDisplayName() + 
                          " activated speed down effect for " + targetPlayer.getDisplayName());
        
        // 현재 속도 저장
        originalSpeed = context.getGameInterface().getPlayerFallSpeed(targetPlayer);
        
        // 새 속도 적용
        context.getGameInterface().setPlayerFallSpeed(targetPlayer, SPEED_DOWN_VALUE);
        context.getGameInterface().setPlayerSpeedItemActive(targetPlayer, true);
        
        isActive = true;
        
        System.out.println("✅ Speed changed: " + originalSpeed + "ms → " + SPEED_DOWN_VALUE + "ms");
    ...
    public void deactivate() {
        ...
        if (context != null) {
            ...
            System.out.println("🔄 Speed DOWN effect ended for " + targetPlayer.getDisplayName() + 
